...
> 基线版本：v0.4.0  
> 文档状态：v0.5 地基重构、v0.6 生活/日程、v0.7 基础工作包与 v0.8 复杂工作包已实现；v0.9 稳定性阶段仍为规划
> 目标：在继续 LLM、论文实验和评测之前，建立可扩展、可测试、低耦合的基础女仆模组架构。

v0.8 的实际代码结构、兼容字段、状态机和当前限制见 [ARCHITECTURE.md](./ARCHITECTURE.md)。本路线中的基础与复杂工作、通用物流和成长曲线已经落地，下一阶段集中处理稳定性与论文前冻结。
...
v0.6 已在该边界上实现生命周期、日程、主手背包迁移和通用拾取；v0.7 又加入可注册持续工作、公共动作和防御中断，仍未把具体工作分发写回实体。

## 24. v0.7 已完成的代码改动

1. 已创建 `MaidWorkMode`、`MaidWorkRule`、冻结的 `MaidWorkRegistry` 和 `MaidWorkController`；
2. 已实现每 tick 有预算的搜索、导航、动作前复验和冷却通用状态机；
3. 已实现普通作物、甘蔗、瓜类、可可、花草、除雪六类农务规则；
4. 已实现烟熏蜂巢采集、原版剪毛、挤奶、喂主人和动物繁殖规则；
5. 已实现低亮度插火把和工作区灭火规则；
6. 已实现 `OFF`、`SELF_DEFENSE`、`DEFEND_OWNER` 三种防御策略；
7. 已实现剑/斧近战与消耗真实弹药、生成真实箭实体的弓箭战术切换；
8. 战斗已作为临时中断接入 `MaidBehaviorController`，不会替换活动契约或丢失任务阶段；
9. 已扩展公共动作层，统一背包预演、原版战利品、补种、放置、实体交互、远程攻击和真实掉落物跟踪拾取；
10. 工作已强制受 WORK 日程、工作地点、活动半径、区块加载、工具、背包和 `mobGriefing` 约束；
11. 已加入命令、GUI、双语资源、NBT 数据版本 3 和工作/防御回归测试；
12. 已修复正式任务命令的命名空间资源标识符解析，并完成真实开发客户端中的农业、边界、采集、防御中断与任务恢复验收；
13. 砍整树、挖矿、熔炉、钓鱼和完整成长当时明确保留到 v0.8，没有在基础工作包中写空实现。

## 25. v0.8 已完成的代码改动

1. 已将 `LUMBERJACK`、`MINER`、`SMELTER`、`FISHER` 注册到同一个 `MaidWorkRegistry`，实体不包含具体工作分发；
2. 已实现 `NaturalTreeAnalyzer` 和 `NaturalTreePolicy`，用有界连通分析、根/树叶阈值、方块实体与玩家木制组件检测保守批准自然树；
3. 整树批准列表可持久化，导航锚定可达树根，真实破坏顺序保持树冠到根部，模式切换时清理计划；
4. 已实现 `MiningSafety`，只批准八类已暴露原版矿石，要求正确镐等级、水平安全站位并拒绝相邻岩浆；
5. 已实现 `FurnaceWorkRule` 的输入选择、熔炉搜索、导航、租约、插入、等待、收取和冷却状态机；
6. 熔炉批次保存维度、位置、租约 ID、输入/燃料/产物和计时，重启后重新验证并通过守恒检查避免覆盖玩家改动；
7. 已实现女仆专属真实浮标实体、Mixin 咬钩桥接和客户端渲染，以及找水、岸边导航、抛竿、等待、收竿和拾取状态机；
8. 已抽取 `FishingSiteGeometry`，显式区分水面、岸上站位与抛竿水道，解决真实岸线的高度错误；
9. 已加入 `TRANSFER_ITEM`、正式 `/maid transfer` 命令、任务定义/验证器/快照和通用单箱精确物流；
10. 已完成 `MaidGrowthController` 与 `MaidGrowthProgression`，工作、战斗、喂食和经验提供受限奖励，等级只给予温和数值增益而不锁工作；
11. 已把 NBT 数据版本升级到 4，并为复杂工作最小稳定阶段、成长奖励窗口和工作首完成位图增加兼容持久化；
12. 已把 `mobGriefing` 从全局持续工作门禁改为规则能力声明，熔炉和钓鱼不再被无关阻止；
13. 已完成 81 项单元测试和完整 Gradle 构建，并在真实开发客户端中通过整树保护、危险矿石拒绝、熔炉跨进程恢复、精确物流和多轮真实钓鱼验收；详见 [V0_8_GAME_TEST_REPORT_ZH.md](./V0_8_GAME_TEST_REPORT_ZH.md)。
