...
> 基线版本：v0.4.0  
> 文档状态：v0.5 地基重构、v0.6 生活/日程、v0.7 基础工作包、v0.8 复杂工作包与 v0.9 制作/工作补给已实现；v0.10 稳定性阶段仍为规划
> 目标：在继续 LLM、论文实验和评测之前，建立可扩展、可测试、低耦合的基础女仆模组架构。

v0.9 的实际代码结构、兼容字段、状态机和当前限制见 [ARCHITECTURE.md](./ARCHITECTURE.md)。本路线中的基础与复杂工作、通用物流、成长曲线、制作动作和工作物资准备已经落地，下一阶段集中处理稳定性与论文前冻结。
...
### Phase 5 / v0.9：制作与工作补给（已完成）

- GUI 具体工作直选与固定宽度分页；
- 菜单内真实 2×2 制作区；
- 服务端原版普通配方规划与原子制作；
- 工作台有界搜索、制作、安全放置和 3×3 制作；
- 伐木徒手降级；
- 斧、镐、锹、剪刀、瓶、桶、火把和钓竿等声明式工作物资补给。

完成条件：

- 客户端不能指定任意产物，配方和结果由服务端权威计算；
- 2×2 与 3×3 网格边界严格区分，制作失败不消耗部分原料；
- 工作台搜索和放置服从工作地点、已加载区块、碰撞和 `mobGriefing`；
- 缺斧伐木可安全徒手降级，其余必须工具的工作不伪造工具；
- 配方、工作台和准备状态机不耦合进具体工作规则。

### Phase 6 / v0.10：稳定性和论文前冻结
...
21. 2×2 与 3×3 配方边界、递归中间材料和容器剩余物守恒；
22. 工作台在搜索、导航和制作过程中消失或被替换；
23. 工作台无合法放置点、`mobGriefing=false` 或背包在制作前被填满；
24. 伐木缺斧/缺材料时徒手降级，取得新材料后重新尝试制作。
...
1. 已在固定 382×230 菜单中加入 2×2 输入/结果槽、生活/日程与具体工作切换，以及三页全部工作直选按钮；
2. 已实现 `CraftItemAction`，只索引服务端普通有形/无形制作配方，支持深度有界的中间材料规划；
3. 每次制作先复制储物区并完整复验原料、配方、产物空间和剩余物，成功后才原子写回；
4. 已实现 `WorkSupplyRequirement`，让具体规则只声明可用谓词、候选产物和是否允许降级；
5. 已实现 `MaidWorkSupplyController` 的个人制作、工作台搜索、工作台制作/放置、导航、3×3 制作和失败冷却状态机；
6. 已为伐木、挖矿、除雪、采蜜/剪毛、挤奶、插火把和钓鱼接入自动物资准备；
7. 伐木在没有斧和可行制作链时通过安全空手租约降级，且动作冷却提高到 60 tick；
8. 数据版本保持 4，菜单制作格和物资准备阶段不持久化，重启后从权威背包/世界状态重新规划；
9. 已完成 86 项单元测试和完整 Gradle 构建，并在真实开发客户端通过按钮、2×2 制作、徒手伐木、自动工作台/木斧、自动剪刀/剪毛和已有工作台/石镐/采煤验收；详见 [V0_9_GAME_TEST_REPORT_ZH.md](./V0_9_GAME_TEST_REPORT_ZH.md)。
