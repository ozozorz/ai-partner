...
本文档描述当前自然语言模块的生产实现。历史实验协议、评测器和模型网关已退出生产代码；当前边界只由 v3 协议与服务器运行时定义。
...
6. `FOLLOW` / `STAY` / `RETURN_HOME` 是持续移动指令，只能作为计划末步；其中回家的即时回执表示“指令已激活”，不是“已经到达”；
...
每个女仆持久化最近 12 条、每条最多 512 字符的用户/助手消息。不会保存 API Key、HTTP 原始请求或模型原始响应。主人可用 `/maid driver clear-memory` 清空当前女仆的记忆；清理前会取消该玩家仍在途的旧模型响应，避免它稍后重新写回上下文。
...
- 即使女仆被配置为 `LLM`，模型端点未就绪时，能被确定性规则明确识别的单意图会走离线降级；开放式对话和复杂计划会安全拒绝；
- HTTP 请求已经发出后若超时、网络失败或输出非法，不会再用另一个本地动作替代执行；
...
自动测试覆盖 v3 Schema/codec 对齐、全部 sealed intent 的动作契约覆盖、计划上限和持续指令位置、重规划目标保持、分阶段响应约束、对话记忆边界、任务契约执行锚点往返，以及任务、工作、GUI、背包和旧世界迁移。真实客户端测试应覆盖 UI/命令离线路径、复杂计划的逐步推进、失败重规划、终态角色回应和保存重进。
