...
> 当前 LLM 控制面已经升级为 v3“连续对话 + 有界语义工作流 + 结果约束对话”；详见 [`LLM_BOUNDED_WORKFLOW_ZH.md`](LLM_BOUNDED_WORKFLOW_ZH.md)。下文自然语言部分保留为 v0.10 单意图基线，其余实体、任务、生活与工作架构仍有效。
...
`AiPartnerDataVersion = 6`。第 6 版为有限任务增加完整契约谓词、契约级恢复计数和任务快照中的剩余超时 tick；旧存档缺少这些字段时使用兼容迁移路径。制作网格与物资准备状态仍为瞬时状态。复杂工作、成长字段与既有生活字段如下：
...
v0.5 的契约、任务快照和兼容字段继续读取；当前写入格式会保存前置条件、目标谓词、不变量、恢复预算和剩余超时，不再在重启后重置为通用谓词或完整超时。`TRANSFER_ITEM` 使用独立任务 ID 和版本化快照，但复用通用存箱执行器。生活配置和有限任务彼此独立，因此任务重启恢复不会覆盖日程地点，日程切换也不会重置任务进度。熔炉状态加载后不会盲信保存数据，会在下一服务端 tick 复验维度、熔炉、租约、配方与物品守恒。
...
- v2 JSON Schema 与 Java codec 的 28 组有效/无效差分语料；
- LLM 远程 HTTPS、失效关闭、完整请求容量、契约级恢复上限、剩余超时和完整契约存档往返。
