# AI Partner v0.8 架构说明

## 1. 目标与范围

v0.8 在 v0.7 的持续工作框架上加入“复杂工作包”。本版本不扩张 LLM 或实验能力；它以保守安全策略实现自然树、暴露矿石、原版熔炉和真实钓鱼，并把通用物流与完整成长效果接入既有任务、生活和持久化边界。

本版本保留 v0.7 的生命周期、基础工作与防御能力，并正式新增：
...
- 普通工作规则只声明目标谓词和原子动作，扫描、导航、冷却和地点边界由 `MaidWorkController` 统一管理；熔炉、钓鱼通过同一规则接口管理自己的多阶段执行，但仍接受统一仲裁和边界暂停；
...
| 公共动作 | `MaidActions` 及 `core.action` | 原版掉落、耐久、背包预演、放置、交互和真实箭实体 |
...
`AiPartnerDataVersion = 4`。v0.8 复杂工作、成长字段与既有生活字段如下：
...
完整 `build` 会同时编译服务端、客户端、Mixin 和测试。v0.8 已启动真实开发客户端，在服务端世界 tick 中验收自然树与玩家结构保护、安全/危险矿石分流、原版熔炉跨进程恢复、任意物品精确搬运、真实浮标多轮钓鱼和成长状态同步；实机还暴露并推动修复了树冠导航和岸线高度两个仅靠单元测试不易发现的问题。过程与结果见 [V0_8_GAME_TEST_REPORT_ZH.md](./V0_8_GAME_TEST_REPORT_ZH.md)。覆盖全部方块/实体变体的自动 GameTest 仍计划放在稳定性里程碑，单元测试不冒充游戏内集成测试。
...
v0.9 将集中处理自动 GameTest、长时间运行、性能预算、存档迁移审计和论文前行为冻结。长期路线见 [FOUNDATION_ARCHITECTURE_ROADMAP_ZH.md](./FOUNDATION_ARCHITECTURE_ROADMAP_ZH.md)。

## 15. 冻结实验说明

v0.4 的数据、Prompt、Schema、场景和结果仍保持原样。v0.8 是基础模组开发版本，实体、运行时、战斗和世界动作字节码已经变化，因此不应把它直接当成 v0.4 冻结实验的等价执行制品。下一轮论文实验必须在玩法地基稳定后重新冻结候选、实现指纹和预注册边界。
