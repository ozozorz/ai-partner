...
当前代码版本为 v0.10“自然语言双驱动”里程碑。v0.4 实验代码仍可用于兼容旧产物，但旧 `JobSpec` schema 不再约束实际玩法入口；新的 v2 控制意图覆盖基础任务、工作、日程、战斗策略与生活配置。基础模组已经具备：
...
v0.11 计划转入稳定性与论文前冻结：补充自动 GameTest、真实多女仆与长时间运行场景，审计 LLM 成本/隐私边界、制作搜索性能、存档迁移与异常恢复；新的实验系统将在 v2 控制意图之上重新设计，不复用 v0.4 冻结协议作为玩法限制。
