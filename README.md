...
当前代码版本为 v0.5 基础架构里程碑；v0.4 实验数据与协议继续作为冻结基线保留：
...
- 保持 v0.4 冻结产物不变，在 v0.5 中统一 JSON Schema 与本地解码语义；
- 冻结一次模型候选并在全部架构变体中回放，消除独立模型调用混杂；
- 拆分执行前验证、运行时检测与本地恢复，并扩展独立扰动模板和安全事件判定；
- 增加核心状态机、持久化、配置安全与 GameTest/集成测试；
- 按韩国游戏学会模板完成韩文或英文论文、图表、参考文献与相似度检查。
